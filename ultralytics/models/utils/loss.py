...
class DETRLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_mal=True, # CVPR2025-DEIM MAL
                 use_shape=False, 
                 use_uni_match=False,
                 uni_match_ind=0):
        """
        ...
        if loss_gain is None:
            loss_gain = {'class': 1, 'bbox': 5, 'giou': 2, 'no_object': 0.1, 'mask': 1, 'dice': 1, 'mal':1}
        self.nc = nc
    ...
    def _get_mal_loss(self,  pred_scores, targets, gt_scores, num_gts, postfix="", values=None, gamma=1.5, mal_alpha=None):
        """Computes the classification loss based on predictions, target values, and ground truth scores."""
        name_class = f'loss_class{postfix}'
        bs, nq = pred_scores.shape[:2]
        # one_hot = F.one_hot(targets, self.nc + 1)[..., :-1]  # (bs, num_queries, num_classes)
        ...
        gt_scores = gt_scores.pow(gamma)
        if mal_alpha is not None:
            weight = mal_alpha * pred_scores.sigmoid().pow(gamma) * (1 - one_hot) + one_hot
        else:
            weight = pred_scores.sigmoid().pow(gamma) * (1 - one_hot) + one_hot
        with torch.cuda.amp.autocast(enabled=False):            
            loss_cls = (self.vfl(pred_scores, gt_scores, one_hot) *
                    weight).mean(1).sum()
        loss_cls /= max(num_gts, 1) / nq
        return {name_class: loss_cls.squeeze() * self.loss_gain["mal"]}
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=''):
        ...
        elif self.use_shapeiou:
            loss[name_giou] = 0.5*(1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, ShapeIoU=True, EIoU=False))
        else:
    ...
    def _get_loss(self,
        ...
        # balanceweight
        # ls_score = torch.sigmoid(torch.max(pred_scores, dim=-1, keepdim=True)[0])
        # match_quality = self.alpha * torch.mean(iou)
        # loss_weights = torch.clamp(match_quality, min=0.1, max=1.0)

        loss = {}
        loss.update(self._get_loss_class(pred_scores, targets, gt_scores, len(gt_bboxes), postfix))
        
        # loss.update(self._get_mal_loss(pred_scores, targets, gt_scores, len(gt_bboxes), postfix))
