...
from ultralytics.utils.metrics import bbox_iou, WiseIouLoss
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, pred_bboxes, pred_scores, gt_bboxes, gt_cls, gt_groups, masks=None, gt_mask=None):
        ...
        if self.use_wiseiou:
            cost_giou = self.wiou_loss(pred_bboxes.unsqueeze(1), gt_bboxes.unsqueeze(0), ret_iou=False, ratio=0.7, d=0.0, u=0.95)
        else:
            cost_giou = 1.0 - bbox_iou(pred_bboxes.unsqueeze(1), gt_bboxes.unsqueeze(0), xywh=True, GIoU=True, ShapeIoU=False,  EIoU=False).squeeze(-1)
